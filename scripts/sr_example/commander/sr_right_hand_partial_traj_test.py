...
import rospy
from control_msgs.msg import JointTrajectoryControllerState, FollowJointTrajectoryActionResult, FollowJointTrajectoryActionGoal
...
class PartialTrajListener():
    def __init__(self):
        ...
        rospy.Subscriber("/rh_trajectory_controller/state", JointTrajectoryControllerState, self.callback)
        rospy.Subscriber("/rh_trajectory_controller/follow_joint_trajectory/result", FollowJointTrajectoryActionResult, self.callback_result)
        rospy.Subscriber("/rh_trajectory_controller/follow_joint_trajectory/goal", FollowJointTrajectoryActionGoal, self.callback_goal)
    ...
    def callback(self, state):
        ...
        if self.start_goals and not self.finish_goals:
            ...
            self.mfj3_vel_actual.append(state.actual.velocities[self.mfj3_index])
            self.ffj3_vel_desired.append(state.desired.velocities[self.ffj3_index])
            self.mfj3_vel_desired.append(state.desired.velocities[self.mfj3_index])
...
#         self.finish_time_goals.append(result.header.stamp.to_sec())
    ...
    def graph(self):
        ...
        for i,traj in enumerate(self.trajectories):
            ...
            mfj3_goal_index = self.goal_joint_names[i].index("rh_MFJ3") if "rh_MFJ3" in self.goal_joint_names[i] else -1

            for point in traj:
                if ffj3_goal_index > -1:
                    time_ffj3_traj.append(point.time_from_start.to_sec()+(self.start_time_goals_trajectory[i] - time_cero))
                    angle_ffj3_traj.append(point.positions[ffj3_goal_index])
                if mfj3_goal_index > -1:
                    time_mfj3_traj.append(point.time_from_start.to_sec()+(self.start_time_goals_trajectory[i] - time_cero))
                    angle_mfj3_traj.append(point.positions[mfj3_goal_index])

            if ffj3_goal_index > -1:
                plt.subplot(3,2,1)
                plt.plot(time_ffj3_traj, angle_ffj3_traj, 'o', label="Traj " + str(i+1))

            if mfj3_goal_index > -1:
                plt.subplot(3,2,2)
                plt.plot(time_mfj3_traj, angle_mfj3_traj, 'o', label="Traj " + str(i+1))
        ...
        plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=5, mode="expand", borderaxespad=0.,prop={'size':8})
        ...
        self.plot_settings(plt)
        plt.ylim(ymax = 2.2, ymin = -0.1)
        plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=4, mode="expand", borderaxespad=0.,prop={'size':8})
...
keys = ['rh_FFJ1', 'rh_FFJ2', 'rh_FFJ3', 'rh_FFJ4', 'rh_THJ4', 'rh_THJ5', 'rh_THJ1', 'rh_THJ2', 'rh_THJ3', 'rh_LFJ2', 'rh_LFJ3', 'rh_LFJ1', 'rh_LFJ4', 'rh_LFJ5', 'rh_RFJ4', 'rh_RFJ1', 'rh_RFJ2', 'rh_RFJ3', 'rh_MFJ1', 'rh_MFJ3', 'rh_MFJ2', 'rh_MFJ4', 'rh_WRJ2', 'rh_WRJ1']
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        if len(listener.joints_time)>5 and (listener.joints_time[-1] - listener.joints_time[0]) > 15.0 and not graphs_finished:
