...
hand_joints_state = {
    k: (v * scale) for k, v in all_joints_state.items() if k.startswith(prefix + "_") and not k.startswith(prefix + "_W")}
arm_joints_state = {
    k: (v * scale) for k, v in all_joints_state.items() if k.startswith(prefix[0] + "a_") or k.startswith(prefix + "_W")}
