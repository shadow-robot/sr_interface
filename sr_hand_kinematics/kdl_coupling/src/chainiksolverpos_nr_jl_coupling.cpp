  ...
  int ChainIkSolverPos_NR_JL_coupling::CartToJnt(const JntArray &q_init, const Frame &p_in, JntArray &q_out)
  {
    ...
    for (i = 0; i < maxiter; i++)
    {
      ...
      if (iksolver_wdls != NULL)  // Verify Mx matrix if ik velocity solver is based on WDLS
      {
        ...
        if (Mx(0, 0) == 0.0)
        {
          delta_twist_temp.vel.x(0.0);
        }
        if (Mx(1, 1) == 0.0)
        {
          delta_twist_temp.vel.y(0.0);
        }
        if (Mx(2, 2) == 0.0)
        {
          delta_twist_temp.vel.z(0.0);
        }
        if (Mx(3, 3) == 0.0)
        {
          delta_twist_temp.rot.x(0.0);
        }
        if (Mx(4, 4) == 0.0)
        {
          delta_twist_temp.rot.y(0.0);
        }
        if (Mx(5, 5) == 0.0)
        {
          delta_twist_temp.rot.z(0.0);
        }
      }
