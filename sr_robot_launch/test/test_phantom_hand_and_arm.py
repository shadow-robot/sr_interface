...
class TestHandAndArmSim(TestCase):
    ...
    def test_1_autodetection_right_hand(self):
        ...
        hands_detected = open_yaml('/tmp/sr_hand_detector.yaml')

        hand_in_config = False
        for hand_serial, hand_eth in hand_to_confirm.items():
            if hand_serial in hands_detected.keys() and hands_detected[hand_serial] == hand_eth:
                hand_in_config = True
    ...
    def test_12_hand_and_arm(self):
        ...
        joint_value_diff_arm_and_hand = joints_error_check(hand_and_arm_joints_target,
...
def open_yaml(path):
    with open(path) as yml_file:
        hands_config = yaml.load(yml_file, Loader=yaml.FullLoader)
    return hands_config
